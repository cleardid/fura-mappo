...
- 稳定标签：`wp01a-stable`
- 后续兼容要求：WP-01B 必须复用本规范的数据模型、RNG 和状态语义。
...
本规范对应的实现已通过独立补丁审查、GitHub Actions 和 A100 验收。WP-01B 原则上不得修改字段、状态语义、工厂安全原则或输出不可变性。确需变更时，必须先记录兼容性决策并对 WP-01A 回归测试执行完整复验。
