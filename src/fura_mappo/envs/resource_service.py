...
from fura_mappo.envs._movement import _calculate_single_slot_move, _MoveResult
...
class ResourceServiceEnvironment:
    ...
    def _calculate_move(self, current: Position, target: Position) -> _MoveResult:
        """调用共享移动原语，保持 WP-02A 的公开物理语义。"""

        return _calculate_single_slot_move(
            current,
            target,
            self._config.movement_speed,
        )
